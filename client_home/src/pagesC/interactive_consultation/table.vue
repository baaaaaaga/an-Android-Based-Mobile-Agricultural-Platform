<template>
  <view class="page_diy_table page_interactive_consultation_table" :style="{ paddingTop: vuex_custom_bar_height + 'px' }">
    <tn-nav-bar>互动咨询列表</tn-nav-bar>
    <view class="page-list" id="page_diy_table">
        ...
        <view v-for="(o, i) in list" :key="i" class="list-item">
          ...
          <view class="operate-bar">
            <view width="100%" class="me-btn btn-delete" @click="delInfo(i)"
                  v-if="$check_action('/interactive_consultation/table', 'del') || $check_action('/interactive_consultation/view', 'del')">
              删除
            </view>
            <view width="100%" class="me-btn btn-info" @click="$navTo('/pagesC/interactive_consultation/view?' + field + '=' + o[field])"
      ...
      <!--跨表按钮-->
                    ...
                    v-if="$check_action('/consultation_reply/view', 'add')"
                    @click="to_form('/pagesC/consultation_reply/view', o)">
                <span>回复</span>
              </view>
			            </view>
        </view>
