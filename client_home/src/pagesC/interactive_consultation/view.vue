<template>
  <view class="page_diy_view page_interactive_consultation_view" :style="{ paddingTop: vuex_custom_bar_height + 'px' }">
    <tn-nav-bar>互动咨询详情</tn-nav-bar>
    <view id="page_diy_view" class="page-from">
      <uni-forms :modelValue="form" v-if="is_view()" labelWidth="70px">
        ...
        <uni-forms-item v-if="$check_field('get','consultation_number') || ($check_field('add','consultation_number') || $check_field('set','consultation_number'))" label="咨询编号" name="consultation_number">
                      <uni-easyinput type="text" v-model="form['consultation_number']" v-if="(form['interactive_consultation_id'] && $check_field('set','consultation_number')) || (!form['interactive_consultation_id'] && $check_field('add','consultation_number'))"  :disabled="true" />
          <!-- 仅查看 -->
        ...
        <uni-forms-item v-if="$check_field('get','farmers_merchants') || ($check_field('add','farmers_merchants') || $check_field('set','farmers_merchants'))" label="农户商家" name="farmers_merchants">
              ...
              v-if="(form['interactive_consultation_id'] && $check_field('set', 'farmers_merchants')) || (!form['interactive_consultation_id'] && $check_field('add', 'farmers_merchants'))"
			   style="width: 100%;"
          ></uni-data-select>
          <uni-data-select
              v-model="form['farmers_merchants']"
        ...
        <uni-forms-item v-if="$check_field('get','business_name') || ($check_field('add','business_name') || $check_field('set','business_name'))" label="商家名称" name="business_name">
                      <uni-easyinput type="text" v-model="form['business_name']" v-if="(form['interactive_consultation_id'] && $check_field('set','business_name')) || (!form['interactive_consultation_id'] && $check_field('add','business_name'))" :disabled="disabledObj['business_name_isDisabled']" />
          <!-- 仅查看 -->
        ...
        <uni-forms-item v-if="$check_field('get','contact_number') || ($check_field('add','contact_number') || $check_field('set','contact_number'))" label="联系号码" name="contact_number">
              <uni-easyinput type="number" v-model="form['contact_number']" v-if="(form['interactive_consultation_id'] && $check_field('set','contact_number')) || (!form['interactive_consultation_id'] && $check_field('add','contact_number'))" :disabled="disabledObj['contact_number_isDisabled']" />
          <!-- 仅查看 -->
        ...
        <uni-forms-item v-if="$check_field('get','business_address') || ($check_field('add','business_address') || $check_field('set','business_address'))" label="商家地址" name="business_address">
                      <uni-easyinput type="text" v-model="form['business_address']" v-if="(form['interactive_consultation_id'] && $check_field('set','business_address')) || (!form['interactive_consultation_id'] && $check_field('add','business_address'))" :disabled="disabledObj['business_address_isDisabled']" />
          <!-- 仅查看 -->
        ...
        <uni-forms-item v-if="$check_field('get','date_of_consultation') || ($check_field('add','date_of_consultation') || $check_field('set','date_of_consultation'))" label="咨询日期" name="date_of_consultation">
              <uni-datetime-picker v-if="(form['interactive_consultation_id'] && $check_field('set', 'date_of_consultation')) || (!form['interactive_consultation_id'] && $check_field('add', 'date_of_consultation'))" v-model="form['date_of_consultation']" type="date" :disabled="disabledObj['date_of_consultation_isDisabled']"/>
          <!-- 仅查看 -->
        ...
        <uni-forms-item v-if="$check_field('get','consulting_users') || ($check_field('add','consulting_users') || $check_field('set','consulting_users'))" label="咨询用户" name="consulting_users">
              ...
              v-if="(form['interactive_consultation_id'] && $check_field('set', 'consulting_users')) || (!form['interactive_consultation_id'] && $check_field('add', 'consulting_users'))"
			   style="width: 100%;"
          ></uni-data-select>
          <uni-data-select
              v-model="form['consulting_users']"
        ...
        <uni-forms-item v-if="$check_field('get','consulting_content') || ($check_field('add','consulting_content') || $check_field('set','consulting_content'))" label="咨询内容" name="consulting_content">
              <uni-easyinput type="textarea" v-model="form['consulting_content']" v-if="(form['interactive_consultation_id'] && $check_field('set', 'consulting_content')) || (!form['interactive_consultation_id'] && $check_field('add', 'consulting_content'))" :disabled="disabledObj['consulting_content_isDisabled']"/>
          <!-- 仅查看 -->
      ...
      <view class="form-footer" v-if="$check_action('/interactive_consultation/view','set') || ($check_action('/interactive_consultation/view','add') || $check_option('/interactive_consultation/table','examine'))">
...
export default {
  ...
  methods: {
    ...
    successChoose(filePath, type) {
      uni.uploadFile({
        url: this.$fullUrl('~/api/interactive_consultation/upload?'), //仅为示例，非真实的接口地址
        filePath,
        name: 'file',
    ...
    change_file(key_name) {
      ...
      uni.chooseFile({
        ...
        success: (file) => {
          ...
          uploadFilePathApi(
            '/interactive_consultation/upload',
            filePath,
            undefined,
            {
    ...
    change_img(key_name) {
      ...
      uni.chooseImage({
        ...
        success: (file) => {
          const tempFilePaths = file.tempFilePaths;
          const filePath = tempFilePaths[0];
          uploadFilePathApi('/interactive_consultation/upload',filePath,undefined,undefined,
            (task) => {
              task.onProgressUpdate((res) => {
                this.percent = res.progress;
                console.log('上传进度' + res.progress);
    ...
    async get_group_user_consulting_users() {
      ...
      if(json.result && json.result.obj){
        ...
        this.get_user_session_consulting_users(this.form['consulting_users'])
      }
      else if(json.error){
        console.error(json.error);
      }
    ...
    get_user_session_consulting_users(id){
      ...
      this.$get(url, user_id, function(res) {
        if (res.result && res.result.obj) {
          ...
          _this.form["consulting_users"] = _this.user.user_id
