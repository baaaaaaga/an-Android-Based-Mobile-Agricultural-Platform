	...
	<el-row class="rows row2">
		...
		<el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap search_btns">
				...
				<el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
						...
						<el-button v-if="$check_action('/interactive_consultation/table','del') || $check_action('/interactive_consultation/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
			...
			<el-table-column fixed="right" label="操作" min-width="200" v-if="$check_action('/interactive_consultation/table','set') || $check_action('/interactive_consultation/view','set') || $check_action('/interactive_consultation/view','get') 
						|| $check_action('/consultation_reply/table','add') || $check_action('/consultation_reply/view','add') 
						" >
				...
				<template slot-scope="scope">
					...
					v-if="$check_action('/interactive_consultation/table','set') || $check_action('/interactive_consultation/view','set') || $check_action('/interactive_consultation/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
						<!--跨表按钮-->
										<el-button class="e-button el-button--small is-plain el-button--primary" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/consultation_reply/view')" v-if="($check_action('/consultation_reply/table','add') || $check_action('/consultation_reply/view','add')) && !scope.row['consultation_reply_limit']">
						<span>回复</span>
					</el-button>
										</div>
				</template>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/interactive_consultation/get_list?like=0",
				url_del: "~/api/interactive_consultation/del?",
		...
		methods: {
			...
			get_list_after: function get_list_after(res, func, url) {
				let _this = this
													_this.list.map((item) => {
					let param = {
						source_table: "interactive_consultation",
						source_id: item.interactive_consultation_id,
						source_user_id: _this.user.user_id
					};
					if(item.consultation_reply_limit_times > 0){
						_this.$get("~/api/consultation_reply/count?",param,(result)=>{
							if(result){
								if(result.result >= item.consultation_reply_limit_times){
									_this.$set(item,'consultation_reply_limit',true);
								}else{
									_this.$set(item,'consultation_reply_limit',false);
								}
							}
						})
					}else{
